...
2026年9月3日現在、実働版の入口はAIUI Studioへ登録した非公開の公式AIUI Agentです。公開コードにあるAndroid起動役と一時AIXの経路は、それ以前に各層を検証した参考実装であり、現在のAgent配布物そのものではありません。

公式Agentへ移行しても、Mac側の認証、短命な会話接続、Whisper、Codex、Obsidian・Google Driveの安全境界は引き続き使っています。一方、実働AIX、AIUI Studioの個人設定、端末固有の終了補助実装と権限設定は公開していません。このリポジトリだけから現在の実働版をそのまま再現することはできません。
