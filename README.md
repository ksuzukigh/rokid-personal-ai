...
2. 「どうぞ」と表示されたら、Rokidのつるにある操作部分（テンプル）を1回操作して話します。
3. 話し終わったら、テンプルをもう一度操作します。
4. ChatGPTの答えがRokidに表示されます。
5. 続けて話すときは、「続けて話す」を選びます。
6. 終わるときは、テンプルをダブルタップします。

「私のAI」を開いただけでは録音は始まりません。「どうぞ」と表示されたあと、テンプルを操作すると録音が始まります。録音した音声は、文字に変換したあと削除します。
...
「Hi Rokid、私のAI」と呼びかけると、「どうぞ」の画面が開きます。画面表示、テンプル操作、マイク録音にはRokid側の機能を使います。
...
Macへ届いた音声は、whisper.cppで動かすWhisperを使って文字に変換します。Whisperは、音声から文字を作るためのAIです。
...
文字にした発言は、ChatGPTデスクトップアプリに同梱されているCodex CLIへ渡します。Codex CLIは、会話の内容を読み、Webやファイル、接続したサービスを使って依頼を処理するための仕組みです。現在の環境では、AIモデルに **GPT-5.6 Luna** を指定しています。
...
質問や依頼の理解を決まった言い方や固定メニューへ狭めない一方で、通信、資料の読み取り、保存や編集には次の制限をかけています。
...
- 保存や編集は、対象と内容をRokidに表示し、使う人が別の1回操作をした後だけ実行する
...
表示した内容を確認して操作したときだけ、その文書を作ります。取り消した場合、確認できる時間を過ぎた場合、確認後に本文や保存先が変わった場合は、何も変更しません。
